...
class MetabolismWindow(tk.Toplevel):
    ...
    def __init__(self, master, proposition_instance, nick, *args, **kwargs):
        ...
        # self.ax.pie(self.calculate_proportions(self.calories), labels=self.labels, autopct='%1.1f%%', startangle=90)
        #
        # # Dodanie tytułu wykresu
        # self.ax.set_title('Nutrients')
        # self.nutrients_graph = FigureCanvasTkAgg(self.fig, master=self)
        # self.nutrients_graph.draw()
        ...
        # Osadzanie poszczególnych widgetów wewnątrz okna
